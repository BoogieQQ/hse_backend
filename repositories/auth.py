...
class AuthRedisStorage:
    ...
    async def get(self, refresh_token: str) -> str | None:
        async with get_redis_connection() as connection:
            value = await connection.get(self._build_key(refresh_token))
...
class AuthRepository:
    ...
    async def get_user_id_by_refresh_token(self, refresh_token: str) -> int | None:
        user_id_str = await self.redis_storage.get(refresh_token)
